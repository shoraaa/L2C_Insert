...
class VRPEnv:
    ...
    def load_problems(self, episode, batch_size):
        ...
        if not self.test_in_vrplib:
            ...
            if self.sub_path:
                self.solution, self.abs_scatter_solu_1, self.abs_partial_solu_2 = \
                                                self.sampling_subpaths_L2Insert(self.problems, self.solution)
            ...
            self.raw_data_capacity = torch.tensor([problem_capacitys]).cuda().float()
    ...
    def _get_new_data_v2(self, data, selected_node_list, prob_size, B_V):
    ...
    def sampling_subpaths_L2Insert(self, problems, solution):
        ...
        batch_size = problems.shape[0]
        embedding_size = problems.shape[2]
        length_of_subpath = torch.randint(low=4, high=problems_size, size=[1])[0]  # in [4,V]
        # length_of_subpath = 60  # in [4,V]
        # print('------ length_of_subpath ', length_of_subpath)
        # inverse
        solution = self.vrp_whole_and_solution_subrandom_inverse(solution)
        ...
        # 训练阶段先不管按空间排序解，测试阶段再搞

        # 2. 将 solution分割成两部分，一部分是end with depot的连续解片段，剩余的点都是散点
        ...
        select_end_with_depot_node = torch.ones(batch_size,dtype=torch.long)* (problems_size-1)
        ...
        index_2 = torch.arange(batch_size)
        index_3 = index_1 + index_2
        abs_partial_solu_2 = sub_tour[index_3, :, :]
        ...
        # print(abs_partial_solu_2[0])
        # assert False
        # index_3_2 = self._get_new_data_v2()
        ...
        # print(abs_partial_solu_2.shape)
        # print(solution[:,:,0].shape)
        # print(abs_partial_solu_2[:,:,0].shape)
        # print(problems_size)
        abs_scatter_solu_1 = self._get_new_data_v2(solution[:,:,0]-1, abs_partial_solu_2[:,:,0]-1,
                                                   problems_size, batch_size)
        abs_scatter_solu_1 = abs_scatter_solu_1 + 1

        abs_partial_solu_2[:,0,1]=1
        ...
        # 这个 abs_partial_solu_2 需要表示成为一个完整的解。
        # print(abs_partial_solu_2[0])

        # 现在 sample 的解还是 end with depot 的
        # 然后starting point就直接回到depot
        # 在学习和推理过程中，也是选择离上一个被选择的点曼哈顿距离下最近的点。
        # 会mask掉插入后造成subtour exceed capacity的边，为了对齐，只能加mask了。
        #
        ...
        #
        # solution_first_node = solution[:,0,0]
        # print(solution_first_node[0])
    ...
    def sampling_subpaths_by_Proximity(self, problems, solution, length_sub):
        ...
        problems_size = solution.shape[1]
        batch_size = problems.shape[0]
        # embedding_size = problems.shape[2]
        ...
        # print('0 ---------------- solution \n', solution)
        # print('0 ---------------- problems_size \n', problems_size)
        # mm = torch.randint(low=4, high=problems_size, size=[1])[0].item()  # in [0,N)
        # solution = torch.roll(solution, shifts=mm, dims=1)

        selected_node_index = torch.randint(low=1, high=coor.shape[1], size=[1])[0]  # in [4,N]

        # print('1 ---------------- selected_node_index \n', selected_node_index)

        # 这一点对应的坐标
        selected_one_node = coor[:, [selected_node_index], :]

        # print('2 ---------------- selected_one_node \n', selected_one_node)

        # depot点肯定不能当作散点。

        scatter_coor_unsorted = coor[:, 1:, :]

        # 计算所有点距离被选点的距离
        distance = torch.norm(scatter_coor_unsorted - selected_one_node, dim=-1)
        # distance = ploar_distance2(scatter_coor_unsorted,coor[:, [0], :],selected_one_node)
        ...
        #
        # # 归一化
        # normalized_probabilities = inverted_probabilities / inverted_probabilities.sum()

        # selected_solution_index = (1/(sorted_distance+0.0001)).multinomial(length_sub,replacement=False).reshape(batch_size,-1)+1
        #
        # indexxxx = torch.arange(batch_size)[:,None].repeat(1,length_sub)

        # inverted_probabilities = 1 / (distance + 0.0001)
        #
        # # sums = torch.sum(inverted_probabilities,dim=1,keepdim=True)
        # # normalized_probabilities = inverted_probabilities / sums
        # selected_solution_index =inverted_probabilities.multinomial(length_sub,replacement=False).reshape(batch_size,-1)+1
        # #
        # print('3 ---------------- sorted_index \n', sorted_index)

        # # 这个 radius 是用来画图的
        # radius = sorted_distance[0,length_sub - 1]

        # 选择 k-nearest 的 index
        selected_solution_index = sorted_index[:, :length_sub] + 1

        # selected_solution_index1 = sorted_index[:, :int(length_sub/2)] + 1
        #
        # selected_solution_index2 = sorted_index[:, int(length_sub/2) + 10: length_sub+10] + 1
        # selected_solution_index = torch.cat((selected_solution_index1, selected_solution_index2),dim=1)

        # selected_solution_index = sorted_index[indexxxx, sorted_distance_selected] + 1
        # print('4 ---------------- selected_solution_index \n', selected_solution_index)

        # 这个是 node 的 absolute index，即solution里面的值
        # print('---------selected_solution_index shape \n', selected_solution_index.shape)

        unselected_solution_index = self._get_new_data_v3(solution, selected_solution_index, problems_size, batch_size)

        # print('5 ---------------- unselected_solution_index \n', unselected_solution_index)
        # print('---------unselected_solution_index shape \n', unselected_solution_index.shape)

        return selected_solution_index, unselected_solution_index

    def _get_new_data_v3(self, data_, selected_node_list, prob_size, batch_size):

        data = data_.clone()

        # padding_data_length = data.shape[1]


        # print('1---------------- zero_indexs \n', zero_indexs)

        # 2. 然后仅仅排序 customer node
        # 确认了selected_node_list在哪些位置后，仅仅将这些位置赋值-2，然后将其放回原本的index里
        # 再把这些-2的值给剔除出去，这样就形成了剩余的解。

        # print('2---------------- selected_node_list \n', selected_node_list)

        customer_nodes = data[data.gt(0.5)].reshape(batch_size,-1)-1 # customer index是从1开始的，所以这里减1
        selected_node_list = selected_node_list - 1 # customer index是从1开始的，所以这里减1

        customer_node_num = customer_nodes.shape[1]

        # print('2---------------- customer_nodes \n', customer_nodes)



        new_sulution_ascending, rank = torch.sort(customer_nodes, dim=-1, descending=False)  # 升序

        # print('1.1 ---------------- new_sulution_ascending \n', new_sulution_ascending)
        # print('1.2 ---------------- rank \n', rank)

        _, new_sulution_rank = torch.sort(rank, dim=-1, descending=False)  # 升序

        # print('2---------------- new_sulution_rank \n', new_sulution_rank)

        list = selected_node_list

        # print('3---------------- list \n', list)

        new_list = torch.arange(customer_node_num)[None, :].repeat(batch_size, 1)

        # print('4---------------- new_list \n', new_list)

        # new_list_len = customer_node_num - list.shape[1]  # shape: [B, V-current_step]
        ...
        new_list[index_1, index_2] = -2
        # print('5 ---------------- new_list \n', new_list)
        index_3 = torch.arange(batch_size, dtype=torch.long)[:, None].repeat(1, customer_node_num)
        ...
        #
        # unselect_list = unselect_list + 1 # 前面减去了1，所以这里要加1
        # print('7 ---------------- unselect_list \n', unselect_list)

        # 3. 当customer node排序完成后，再把0点插入进入。

        data[data.gt(0.5)]=new_list.ravel()
        # print('8 ---------------- data \n', data)

        unselect_list = data[data.gt(-0.5)].reshape(batch_size,-1)

        # print('9 ---------------- unselect_list \n', unselect_list)
        # 9 ---------------- unselect_list
        #  tensor([[ 0,  0, 16,  7,  0, 20,  0, 15, 14,  2,  8, 11,  0],
        #         [ 0, 18,  0,  2, 14, 10,  0,  4, 19,  0, 15,  8,  0],
        #         [ 0,  0, 20,  4,  2, 11, 19,  0, 13, 17,  3,  0,  0]])
        # 就是，相当于把 重复的每两个0中的一个0往最后挪。

        unselect_list_flag = tran_to_node_flag(unselect_list)
        # unselect_list = node_flag_tran_to_(unselect_list_flag)

        # print('10 ---------------- unselect_list \n', unselect_list)
        # 10 ---------------- unselect_list
        #  tensor([[ 0, 16,  7,  0, 20,  0, 15, 14,  2,  8, 11,  0,  0],
        #         [ 0, 18,  0,  2, 14, 10,  0,  4, 19,  0, 15,  8,  0],
        #         [ 0, 20,  4,  2, 11, 19,  0, 13, 17,  3,  0,  0,  0]])

        return unselect_list_flag
    ...
    def reset(self, mode, sample_size = 1):
        ...
        if mode == 'train':
            ...
            self.step_state = Step_State(data = self.problems, first_node=self.first_node[:, None, :],
    ...
    def drawPic_VRP(self, coor_, order_node_,order_flag_,name='xx',):
        ...
        # order_flag_: shape (V)
        # print(coor_.shape)
        # print(order_node_.shape)
        # print(order_flag_.shape)

        if self.test_in_vrplib:
            print(self.test_in_vrplib,'self.test_in_vrplib')
            coor_ = coor_.clone() * (self.problem_max_min[0] - self.problem_max_min[1]) + \
    ...
    def destroy_solution(self, problem, complete_solution,length_of_subpath):

        abs_scatter_solu_1, abs_partial_solu_2 = self.sampling_subpaths_repair(
            problem, complete_solution,length_of_subpath)

        return abs_scatter_solu_1, abs_partial_solu_2


    def sampling_subpaths_repair(self, problems, solution,length_of_subpath):
        # problems shape (B,V+1,4)
